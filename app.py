...
def mover_para_fluxo(aba_origem, id_linha, descricao, valor, tipo):
    ...
    try:
        ...
        # Aqui no sistema real, marcaríamos o Check na planilha
        st.success(f"Lançamento realizado no Fluxo de Caixa!")
    except:
        st.error("Erro ao processar transação financeira.")

def explosao_estoque(nome_kit, qtd_produzir):
    """Calcula quanto de MP será necessário para a produção desejada"""
    # Carrega as tabelas
    ...
    # Exemplo de lógica simplificada para demonstração na tela
    st.write(f"### Planejamento de Produção: {qtd_produzir}x {nome_kit}")
    # (A lógica completa de recursão Kit->Montagem->Peça entraria aqui)
...
if st.session_state.page == 'Dashboard':
    ...
    with tab1:
        ...
        if st.button("Confirmar Pagamento Selecionado"):
            # Lógica para pegar a linha selecionada e mover
            st.info("Funcionalidade: O sistema moverá este item para o Fluxo de Caixa.")

    with tab3:
        df_fluxo = pd.DataFrame(sh.worksheet("Fluxo_Caixa").get_all_records())
        st.write("#### Histórico de Entradas e Saídas")
        st.table(df_fluxo)

elif st.session_state.page == 'Estoque':
    st.title("📦 Marcenaria - Estoque e Explosão")
    cat = st.selectbox("Categoria", ["MP - Matéria Prima", "Peças", "Montagens", "Kits"])

    if cat == "MP - Matéria Prima":
        df_mp = pd.DataFrame(sh.worksheet("Estoque_MP").get_all_records())
        st.dataframe(df_mp)

    st.write("---")
    st.write("### 🔨 Calculadora de Explosão de Materiais")
    col_k, col_q = st.columns(2)
    kit = col_k.selectbox("Selecione o Kit para Produção", ["Torre de Aprendizagem", "Cubo Sensorial", "Prateleira Lúdica"])
    ...
    for i, row in df_pedidos.iterrows():
        # Lógica de Cores baseada no prazo
        prazo = row['Prazo_Producao']
        cor = "#E8F5E9" # Verde padrão
    ...
    st.write("Marque as etapas concluídas:")
